...
Al finalizar el ciclo de desarrollo correspondiente a esta entrega, centrado principalmente en la construcción del backend de FullTank y su integración con la Web Application desarrollada en sprints previos, el equipo ha llegado a las siguientes conclusiones, contrastando los resultados obtenidos con los planteamientos iniciales del proceso Lean UX y con los objetivos técnicos definidos para los Sprints 3 y 4:

1. Validación de Problem Statements y Supuestos (Assumptions):

Inicialmente, se estableció como Problem Statement que las empresas que gestionan combustible enfrentan ineficiencias operativas debido a procesos manuales, falta de trazabilidad y poca visibilidad en la cadena de suministro. Con la implementación del backend bajo una arquitectura Domain-Driven Design organizada por bounded contexts, y su conexión real con el frontend Angular (reemplazando la Fake API basada en json-server utilizada en el Sprint 2), se confirmó que la digitalización del ciclo completo de pedidos —ahora soportado por servicios reales de autenticación, inventario, pedidos, logística, pagos y notificaciones— reduce significativamente los errores operativos y mejora la trazabilidad respecto al flujo simulado inicial.

Asimismo, se validó el supuesto de que los usuarios están dispuestos a adoptar soluciones digitales siempre que estas simplifiquen sus procesos y centralicen la información, lo cual se sostuvo también durante la etapa de integración backend-frontend, donde los flujos de negocio pudieron ejecutarse de principio a fin sobre datos persistidos en una base de datos relacional real. Sin embargo, el supuesto de que bastaba con exponer los endpoints principales de pedidos fue matizado durante el desarrollo: la necesidad de completar historias técnicas heredadas del Sprint 3 al Sprint 4 (recuperación de contraseña, logout, consulta de pedidos por usuario, actualización de perfil, detalle de solicitud y eliminación de equipos) evidenció que la robustez de la autenticación, las restricciones de autorización y la consistencia de las consultas por usuario/empresa son tan críticas para la experiencia como el registro de pedidos en sí mismo.

2. Contrastación de Hipótesis (Hypothesis Statements):

Hipótesis de Valor para Empresas:
Se planteó que "Si proporcionamos un dashboard centralizado, las empresas podrán optimizar la gestión de combustible y reducir errores en los pedidos". Con el backend ya conectado al frontend, esta hipótesis se sostuvo a nivel de datos reales: los KPIs, historiales y estados de pedido mostrados en el dashboard dejaron de depender de datos simulados y pasaron a reflejar la persistencia real gestionada por los servicios de pedidos e inventario, validando que la centralización de información es sostenible más allá del prototipo.

Hipótesis de Valor para Proveedores:
Se propuso que "Si los proveedores tienen visibilidad de las solicitudes en tiempo real, podrán mejorar sus tiempos de respuesta y coordinación logística". La implementación de los servicios de logística (entregas, conductores, vehículos) y notificaciones sobre una base de datos real confirmó esta hipótesis, evidenciando además que la disponibilidad de recursos y el estado de despacho requieren validaciones de negocio en el propio backend (y no solo en el frontend) para evitar conflictos de asignación.

No obstante, se identificó que ofrecer únicamente información resumida no es suficiente; los usuarios requieren actualizaciones constantes y trazabilidad detallada de cada pedido. Esto se reforzó durante la integración backend-frontend, donde fue necesario documentar formalmente cada servicio mediante OpenAPI/Swagger y completar los endpoints pendientes en el Sprint 4, confirmando que el monitoreo en tiempo real y la exactitud de los contratos de API son requisitos centrales —y no opcionales— para que la propuesta de valor validada en las entrevistas de usuario se sostenga sobre una arquitectura real de producción.
